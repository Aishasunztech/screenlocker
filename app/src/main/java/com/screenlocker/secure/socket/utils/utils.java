...
//                    MyApplxication.getAppDatabase(context).getDao().updateAllApps(false, false, false);
    ...
    public static void refreshApps(Context context) {
        ...
        String installedPackages = prefUtils.getStringPref( INSTALLED_PACKAGES);
                    ...
                    for (ResolveInfo ri : allApps) {
                        if (ri.activityInfo.packageName.equals(packageName)) {
                            ...
                            new Thread(() -> {
                                ...
                                if (finalI == data.length - 1) {
                                    listener.onAppsRefresh();
                                    prefUtils.saveStringPref( INSTALLED_PACKAGES, null);
                                }
                            }).start();
                        }
                    }
...
//                    startActivityForResult(intent, RESULT_ENABLE);
    ...
    public static void saveArrayList(ArrayList<InstallModel> list, Context context) {
        ...
        PrefUtils prefUtils = PrefUtils.getInstance(context);
        prefUtils.saveStringPref( APPS_LIST, null);
        prefUtils.saveStringPref( APPS_LIST, json);
    }

    public static ArrayList<InstallModel> getArrayList(Context context) {
        Gson gson = new Gson();
        String json = PrefUtils.getInstance(context).getStringPref( APPS_LIST);
        Type type = new TypeToken<ArrayList<InstallModel>>() {
        }.getType();
