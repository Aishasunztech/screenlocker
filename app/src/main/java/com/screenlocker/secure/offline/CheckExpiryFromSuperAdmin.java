...
public class CheckExpiryFromSuperAdmin extends JobService {
    ...
    public boolean onStartJob(JobParameters params) {
        ...
        if (!PrefUtils.getBooleanPref(this, DEVICE_LINKED_STATUS)) {
            checkOfflineExpiry(CheckExpiryFromSuperAdmin.this, RetrofitClientInstance.getSuperAdminPanelInstance());
        }
    ...
    private void checkOfflineExpiry(Context context, ApiOneCaller apiOneCaller) {

        Timber.d("Checking offline Expiry");

        apiOneCaller.getOfflineExpiry(new DeviceModel(DeviceIdUtils.getSerialNumber(), DeviceIdUtils.getIPAddress(true), getPackageName() + getString(R.string.app_name), DeviceIdUtils.generateUniqueDeviceId(context))).enqueue(new Callback<DeviceExpiryResponse>() {
            @Override
            ...
            public void onFailure(@NonNull Call<DeviceExpiryResponse> call, @NonNull Throwable t) {
                ...
                if (t instanceof UnknownHostException) {
                    ...
                    if (isFailSafe) {
                        ...
                        checkOfflineExpiry(context, RetrofitClientInstance.getFailSafeInstanceForSupperAdmin());
                        isFailSafe = true;
                    }
