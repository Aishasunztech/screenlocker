package com.screenlocker.secure.views.patternlock;
...
import com.screenlocker.secure.views.patternlock.utils.ResourceUtils;
 ...
 */
public class PatternLockWithDotsOnly extends View {
    ...
    private List<PatternLockWithDotListener> mPatternListeners;
    // The pattern represented as a list of connected {@link Dot}
    private ArrayList<PatternLockWithDotsOnly.Dot> mPattern;
    ...
    public PatternLockWithDotsOnly(Context context) {
        this(context, null);
    }

    public PatternLockWithDotsOnly(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PatternLockWithDotsOnly);
        try {
            sDotCount = typedArray.getInt(R.styleable.PatternLockWithDotsOnly_dotCount1,
                    DEFAULT_PATTERN_DOT_COUNT);
            mAspectRatioEnabled = typedArray.getBoolean(R.styleable.PatternLockWithDotsOnly_aspectRatioEnabled1,
                    false);
            mAspectRatio = typedArray.getInt(R.styleable.PatternLockWithDotsOnly_aspectRatio1,
...
//                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.pattern_lock_dot_size));
            ...
            mDotSelectedSize = (int) typedArray.getDimension(R.styleable
                            .PatternLockWithDotsOnly_dotSelectedSize1,
                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.pattern_lock_dot_selected_size));
            mDotAnimationDuration = typedArray.getInt(R.styleable.PatternLockWithDotsOnly_dotAnimationDuration1,
                    DEFAULT_DOT_ANIMATION_DURATION);
            mPathEndAnimationDuration = typedArray.getInt(R.styleable.PatternLockWithDotsOnly_pathEndAnimationDuration1,
        ...
        mPatternDrawLookup = new boolean[4][3];

        mDotStates = new DotState[4][3];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 3; j++) {
    ...
    public void setUpRandomizedArray() {
    ...
    protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
        ...
        mViewWidth = adjustedWidth / (float) 3;

        int adjustedHeight = height - getPaddingTop() - getPaddingBottom();
        mViewHeight = adjustedHeight / (float) 4;
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        setPattern(PatternViewMode.CORRECT,
                PatternLockUtils.stringToPattern(PatternLockWithDotsOnly.this, savedState.getSerializedPattern()));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        Log.d("Nadeem", "onTouchEvent:"+event.getAction());
    ...
    public void setDotCount(int dotCount) {
        sDotCount = dotCount;
        mPatternSize = 4 * sDotCount;
        mPattern = new ArrayList<>(mPatternSize);
        mPatternDrawLookup = new boolean[4][sDotCount];

        mDotStates = new DotState[4][sDotCount];
        for (int i = 0; i < 4; i++) {
    ...
    public void addPatternLockListener(PatternLockWithDotListener patternListener) {
        mPatternListeners.add(patternListener);
    }

    public void removePatternLockListener(PatternLockWithDotListener patternListener) {
    ...
    private void notifyListenersStarted() {
        for (PatternLockWithDotListener patternListener : mPatternListeners) {
